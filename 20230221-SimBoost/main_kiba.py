...
f1, f2 = open("data/kiba/target_gene_names.txt"), open("data/kiba/drug_PubChem_CIDs.txt")
id1, id2 = [], []
for i in f1: id1.append(i.strip("\n"))
for i in f2: id2.append(int(i.strip("\n")))

sim_targets = pd.read_csv("data/kiba/kiba_target_sim.txt", 
...
transformed_bindings = pd.read_csv("data/kiba/kiba_binding_affinity_v2.txt", sep="\t", header=None,
    names=id1)
transformed_bindings.index = id2
...
for i in id1:  # id1=list of target names
    for j in id2:  # id2=list of drug names
        if np.isnan(transformed_bindings.loc[j, i]): continue
