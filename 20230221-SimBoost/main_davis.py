...
f1, f2 = open("data/davis/target_gene_names.txt"), open("data/davis/drug_PubChem_CIDs.txt")
id1, id2 = [], []
for i in f1: id1.append(i.strip("\n"))
for i in f2: id2.append(int(i.strip("\n")))

sim_targets = pd.read_csv("data/davis/target-target_similarities_WS_normalized.txt", 
    sep=" ", header=None, names=id1)
sim_targets.index = id1

# sim_drugs = pd.read_csv("data/drug_sis.csv", sep=",", header=None, names=id2)
sim_drugs = pd.read_csv("data/davis/drug-drug_similarities_2D.txt", sep=" ", header=None, names=id2)
sim_drugs.index = id2

sim_drugs_g = pd.read_csv("data/davis/drug_jaccard.csv", sep=",", header=None, names=id2)
sim_drugs_g.index = id2

bindings = pd.read_csv("data/davis/drug-target_interaction_affinities_Kd__Davis_et_al.2011.txt", sep=" ", header=None,
    names=id1)
bindings.index = id2
...
drug_graph = igraph.Graph.Adjacency(drug_m)
target_graph = igraph.Graph.Adjacency(target_m)
