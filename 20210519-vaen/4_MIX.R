...
all.TCGA.pred.mat <- read.table("./Output/3/VAEN_CCLE.A.pred_TCGA.txt", as.is = T, header = T)
solid.TCGA.pred.mat <- read.table("./Output/3/VAEN_CCLE.S.pred_TCGA.txt", as.is = T, header = T)
...
write.table(TCGA.pred.mat, file = paste("./Output/4/VAEN_CCLE.MIX.pred_TCGA.txt", sep = ""), quote = F, sep = "\t", row.names = FALSE)

############
all.CCLE.pred.mat <- read.table("./Output/3/VAEN_CCLE.A.pred_CCLE.txt", as.is = T, header = T)
solid.CCLE.pred.mat <- read.table("./Output/3/VAEN_CCLE.S.pred_CCLE.txt", as.is = T, header = T)
...
for (kdrug in 1:length(drugs)) {
    ...
    plot(Ys[ii, 1], all.CCLE.pred.mat[ii, kdrug + 1], main = paste(drug, ", all, PCC = ", format(r, digits = 3), sep = ""), 
        xlab = "Observed CCLE", ylab = "Predicted CCLE (all)")

    ######### solid
    load(paste("./Output/2/1.CCLE.model.list.S.RData", sep = ""))
    model.list[[drug]] -> res.list
    Ys <- res.list$Ys
    which(Ys[, 1] != -9) -> ii
    r <- cor(Ys[ii, 1], solid.CCLE.pred.mat[ii, kdrug + 1])
    plot(Ys[ii, 1], solid.CCLE.pred.mat[ii, kdrug + 1], main = paste(drug, ", solid, PCC = ", format(r, digits = 3), sep = ""), 
...
write.table(CCLE.pred.mat, file = paste("./Output/4/VAEN_CCLE.MIX.pred_CCLE.txt", sep = ""), quote = F, sep = "\t", row.names = FALSE)

############

all.CCLE.pred.mat <- read.table("./Output/3/VAEN_CCLE.A.pred_CCLE.full.txt", as.is = T, header = T)
solid.CCLE.pred.mat <- read.table("./Output/3/VAEN_CCLE.S.pred_CCLE.full.txt", as.is = T, header = T)
...
write.table(CCLE.pred.mat, file = paste("./Output/4/VAEN_CCLE.MIX.pred_CCLE.full.txt", sep = ""), quote = F, sep = "\t", row.names = FALSE)