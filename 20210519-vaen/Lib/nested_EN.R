...
main <- function(X, Y, n_folds = 10, n_train_test_folds = 5, seed = NA, alpha = 0.5, null_testing = FALSE, drug) {
    ...
    if (length(fit) > 0) {
        ...
        if (fit$nzero[best_lam_ind] > 0 & length(nonzero_idx) > 1) {
            ...
            model_summary <- c(
                drug, alpha, fit$nzero[best_lam_ind], fit$lambda[best_lam_ind], R2_avg, R2_sd, cv_R2_avg, cv_R2_sd, training_R2, pval_est,
            ...
            model_summary <- c(
                drug, alpha, 0, fit$lambda[best_lam_ind], R2_avg, R2_sd,
