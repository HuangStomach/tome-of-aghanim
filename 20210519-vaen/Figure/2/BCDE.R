...
ccle.pred <- read.table("../../Output/3/VAEN_CCLE.A.pred_CCLE.txt", header = T, as.is = T)
gdsc.pred <- read.table("../../Output/3/VAEN_GDSC.A.pred_GDSC.txt", header = T, as.is = T, sep = "\t")
...
for (k1 in 1:nrow(one.drugs.match)) {
    for (k in 1:length(cancer.types)) {
        ...
        df <- as.data.frame(cbind(
            x = ccle[ii, one.drugs.match[k1, 2]],
            y = gdsc[ii, one.drugs.match[k1, 3]]
        ))
        df[, 1] <- as.numeric(as.character(df[, 1]))
        df[, 2] <- as.numeric(as.character(df[, 2]))
