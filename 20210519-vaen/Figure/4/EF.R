...
png("./E.MET.CCLE.png", height = 1200, width = 1200)
multiplot(plotlist = list(p1, p3, p2, p4), layout = matrix(1:4, nrow = 2))
dev.off()

###################################################################################################################
drug.ccle <- read.table("../../Output/3/VAEN_GDSC.A.pred_TCGA.txt", header = T, as.is = T, sep = "\t")
cancer.types <- unique(drug.ccle[, 2])
sample.type <- substr(drug.ccle[, 1], 14, 15)
ss <- gsub("\\.", "-", drug.ccle[, 1])
drug.ccle[, 1] <- ss
...
png("F.MET.GDSC.png", height = 1200, width = 1800)
