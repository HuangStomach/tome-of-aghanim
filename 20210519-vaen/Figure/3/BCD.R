...
pdf("C.CCLE.lineage.pdf", width = 5.7, height = 5)
TCGA.cutoff <- 0.05
par(mar = c(1, 6, 10, 4))
...
mtext(text = colnames(m), side = 2, line = 0.3, at = (0:(gene_count - 1)) / (gene_count - 1), las = 1, cex = .8, col = path.col)
gene_count <- nrow(m)

text((0:(gene_count - 1)) / (gene_count - 1), 1.05, srt = 90, adj = 0, cex = .8, labels = rownames(m), xpd = TRUE)
...
gene_count <- nrow(new.m)
-1 / (2 * gene_count - 2) + (0:(gene_count)) / (gene_count - 1) -> tt.x
gene_count <- ncol(m)
-1 / (2 * gene_count - 2) + (0:(gene_count)) / (gene_count - 1) -> tt.y
...
rbind(
    cbind(
        Cancer = names(sensitive.prop),
        Prop = sensitive.prop,
        grp = "Sensitive"
    ),
    cbind(
        Cancer = names(resistant.prop),
        Prop = -resistant.prop,
        grp = "Insensitive"
    )
) -> dat
...
g <- ggplot(dat, aes(x = Cancer, y = Prop, group = grp, fill = grp)) +
    ...
    ggtitle("CCLE model") +
    theme(
        legend.position = c(0.9, 0.9), legend.title = element_blank(), legend.text = element_text(size = 7),
        plot.title = element_text(hjust = 0.5, size = 7),
        panel.background = element_rect(fill = "grey90"), axis.text.y = element_text(size = 6)
    )

ggsave(g, file = "./D.CCLE.PLX4720.1.pdf", width = 4.5, height = 3)

#####
pdf("B.CCLE.PLX4720.2.pdf", width = 5, height = 3)
