...
for (ksigmoid in start:end) {
    ...
    write.table(TCGA.drug.response.mat, file=paste("./Output/2/", ksigmoid,".pred_TCGA.txt", sep=""), quote=F, sep="\t", row.names=FALSE)
    write.table(self.prediction.mat, file=paste("./Output/2/", ksigmoid,".pred_CCLE.txt", sep=""), quote=F, sep="\t", row.names=FALSE)
    save(model.list, file=paste("./Output/2/", ksigmoid, ".CCLE.model.list.RData", sep=""))
}
