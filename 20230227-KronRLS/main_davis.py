...
def load_davis(sis):
    Y = np.loadtxt("./data/davis/drug-target_interaction_affinities_Kd__Davis_et_al.2011.txt")
    Y = -np.log10(Y / (10 ** 9))
    if sis:
        XD = np.loadtxt("./data/davis/drug_sis.csv", delimiter=",")
    else:
        XD = np.loadtxt("./data/davis/drug-drug_similarities_2D.txt")
    XT = np.loadtxt("./data/davis/target-target_similarities_WS_normalized.txt")    
    return XD, XT, Y
