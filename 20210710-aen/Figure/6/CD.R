...
for (kdrug in 1:length(drugs)) {
    ...
    p <- ggplot(dat4plot, aes(x = cancer, y = value, color = group)) +
        geom_boxplot(aes(color = group)) +
        theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjus = 1), plot.title = element_text(hjust = 0.5)) +
        ggtitle(drugs[kdrug]) +
        theme(legend.position = "none") +
        ylab("log(TMB)") +
        xlab("") +
        stat_summary(fun.data = give_n, geom = "text", size = 2)
    print(p)
}

p <- ggplot(dat4plot, aes(x = cancer, y = value, fill = group)) +
    geom_boxplot(aes(color = group))
print(p)
dev.off()